import {CheckOutlined, MehOutlined, SyncOutlined} from '@ant-design/icons';
...
const AccountConfirm = query => {
	...
	const renderStatus = _ => {
		...
		if (result && result.data && result.data.statusCode === 404) {
			return (
				<>
					<h1 style={{color: 'red'}}>No encontrado <MehOutlined/></h1>
			...
			return (
				<>
					<h1 style={{color: 'orange'}}>Verificado <CheckOutlined/></h1>
			...
			return (
				<>
					<h1 style={{color: 'green'}}>¡Todo listo! <CheckOutlined/></h1>
					<h3>{result.data.name} &lt;{result.data.email}&gt;</h3>
					<p>Te haz dado de alta en nuestra plataforma, ahora puedes <Link href="/"><a>Iniciar Sesión</a></Link> en
						UMANA</p>
				</>
			)
		}
	...
	return (
		<Actions pageTitle="Confirmar cuenta">
			{
				loading
					? <h1><SyncOutlined spin/> Verificando...</h1>
					: renderStatus()
			}
		</Actions>
	)
