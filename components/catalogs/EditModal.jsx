  ...
  return (
    <Drawer placement="right" closable={true} onClose={() => onReset()} visible={visible} width={600} title={title} destroyOnClose={true}>
      <div className="umana-drawer">
        ...
        {data && data.name ? (
          <Form onFinish={onFinish} initialValues={data} validateTrigger="onBlur" onValuesChange={onValuesChange}>
            <Form.Item
              label="Título"
              name="name"
              rules={[
            ...
            {/*Update warning*/}
            {active && (
              <Alert message="Nota" description="Toma en cuenta que si desactivas una categoría padre, también estás desactivando las subcategorías contenidas en él." type="warning" showIcon />
            )}
