...
const SingleJobData = ({job, company}) => {
	return (
		<>
				...
				{job.languages ? (
					<div className="umana-content__item item-lg">
						...
						{job.languages.map((e, idx) => (
							...
							>
								<p>{e.language}</p>
								<Progress width={150} strokeWidth={2} strokeColor="#585858" type="circle" percent={e.comprehension}
								          format={percent => `Comprensión ${percent}%`}/>
								<Progress width={150} strokeWidth={2} strokeColor="#585858" type="circle" percent={e.write}
								          format={percent => `Lectura ${percent}%`}/>
								<Progress width={150} strokeWidth={2} strokeColor="#585858" type="circle" percent={e.speak}
			...
			{/* company info */}
			{company ? (
				<Can I="view" a="COMPANY_INFO_BLOCK">
				...
				<Can I="view" a="COMPANY_INFO_BLOCK">
					<div className="umana-content" style={{marginTop: 80}}>
						{job && job.company && job.company_state === 'public' ? (
							<div className="umana-section-contenct">
								<div className="section-avatar">
									<Avatar icon={<i className="material-icons">location_city</i>} src={job.company.avatar || ''}
									        size={120}/>
								</div>
								<div className="section-title">
